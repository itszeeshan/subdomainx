...
> **Important**: All notification credentials must be set via environment variables — they are never stored in config files or passed via CLI flags. See [CLI Reference](/cli-reference#notification-options) for the full list of environment variables.
