  ...
  scan:
    ...
    steps:
      ...
      - name: Run scan
        ...
        run: |
          subdomainx --subfinder --crtsh --format json --output results/ target.com
      ...
      - name: Send results via email
        ...
        with:
          ...
          subject: "SubdomainX Scan Results - $(date +'%Y-%m-%d %H:%M')"
          to: you@example.com
          from: SubdomainX Bot
          body: "Scan completed. Results attached."
          attachments: results/*
```
...
# Run the scan
subdomainx --subfinder --crtsh --format json \
  --name "scan_${TIMESTAMP}" \
  --output "${OUTPUT_DIR}" \
  "${TARGET}"

# Email results
RESULT_FILE=$(ls -t "${OUTPUT_DIR}"/scan_${TIMESTAMP}* 2>/dev/null | head -1)
if [ -n "${RESULT_FILE}" ]; then
  mail -s "SubdomainX Results - ${TIMESTAMP}" \
    -A "${RESULT_FILE}" \
    "${EMAIL}" < /dev/null
fi
```
